...
local function downloadFile(path, func)
	if not isfile(path) then
		local suc, res = pcall(function()
			return game:HttpGet(
				'https://raw.githubusercontent.com/SolentraXminishakk/BananaVape/'
					.. readfile('bananavxpe/profiles/commit.txt')
					.. '/'
					.. select(1, path:gsub('bananavxpe/', '')),
				true
			)
		end)
...
local drawingactor = loadstring(downloadFile('bananavxpe/libraries/drawing.lua'), 'drawing')(...)
...
if not select(1, ...) and game.PlaceId == 5938036553 then
	if run_on_actor and getactors then
		...
		task.spawn(function()
			...
			local executionString = "loadfile('bananavxpe/main.lua')(" .. drawingactor .. ')'
